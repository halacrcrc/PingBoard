...
本版**只动设置对话框**，程序主体功能与 v1.1.4 完全一致。

- **修复：事件保存目录不显示具体默认路径。** 此前只写「（默认：配置目录）」，看不出文件究竟落在哪。
  现直接显示完整的默认绝对路径（`…\com.pingboard.desktop\pingboard-events.jsonl`），
  处于默认值时另标「默认」小标；路径过长时截断显示、悬停可见全文。
- **修复：卡片右边缘参差。** 根因是中间卡片位于滚动容器**内部**、纵向滚动条额外占去约 9.6px。
  改法：把滚动条槽位固定为 12px（`scrollbar-gutter: stable`），标题区与底部区按同一宽度补位，
  不再依赖浏览器默认滚动条宽度。
- **修复：关闭按钮 ✕ 嵌在「设置」标题卡片内部。** 现已移到弹窗**右上角**（卡片之外），
  与内容区右边缘对齐。笔画同时改由**自绘矢量图形**控制 —— 原来的 `✕` 是字体字符，
  加粗与否取决于字体自身是否有粗体变体，跨机器不可控。
- **修复：「界面与行为」与「并发」两卡下边缘不齐**（原来相差 42.25px，矮的那张下方留一块空白）。
  把「并发」的整段说明移到两卡下方做成整宽一行，两卡等高；说明信息未删。
- **修复：「事件日志」区域排版凌乱。** 原先是 3 列里塞「标签最左 + 提示推到最右」，
  中间大片空白，且「总开关」「写入 jsonl」「超出丢弃最旧」这类提示本身冗余；
  现改为 **2×2 成组块**（标签在上、控件在下），各行列左边缘对齐。
- **改善：默认窗口下无需滚动即可看全。** 弹窗高度上限由 `86vh` 放宽到 `calc(100vh - 2rem)`，
  并轻量压缩纵向节奏（卡片内边距、卡片间距各收 0.5 档）；**字号与控件高度一律未变**。
- 新增 8 个「默认事件文件路径推导」单元测试（Windows 绝对路径 / 正斜杠 / UNC /
  中文与空格目录名 / 尾部斜杠 / 空串等边界），前端纯函数测试 29 → 37。
...
- **修复：清空某主机时归档里的记录没被清掉。** 此前只重写主文件，与上一条修复叠加后会表现为
  「清空了又自己回来」；现已同时清理两个文件，全部清空时不留 0 字节残件。
