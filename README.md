...
- 文件超过 **8 MiB** 自动轮转为 `pingboard-events.1.jsonl`；启动时只读文件尾部用于恢复历史，
  并按主机归属回填（历史条目视为已读，不计入红点）。
- 目录不可写时**静默降级**：应用照常运行、事件仍在界面显示，只是不落盘。

**导出**：日志区可「复制」为纯文本或「导出 CSV」。CSV 为 7 列
（`序号, 时间(本地), 主机, 备注, 事件类型, 等级, 说明`），带 **UTF-8 BOM**，Excel 直接打开中文不乱码。
