...
- **新增：界面缩放。** 设置 →「界面与行为」新增「界面缩放」，提供 **90% / 100% / 110% / 125%**
  四档，等比缩放整个界面（文字、按钮、间距），类似浏览器缩放；默认 100%。
  缩放下「导出」等下拉菜单的定位已按缩放因子修正，不会错位。
- **新增：界面字体自定义。** 设置 →「界面与行为」新增「界面字体」，下拉列出**本机已安装的
  系统字体**（自动枚举、去重、排序），默认「系统默认」即原有字体栈；枚举失败时仅剩默认项，
  不影响其他功能。
- **修复：「未开始」状态徽标折成两行。** 主机列表空间紧张时状态徽标会按字折行
  （「未开」/「始」），违反单行原则 —— 现已锁定单行，并在最窄 1000px 视口下实测零溢出。
- **修复：「清空统计」后未读红点残留。** 问题主机的「有新故障事件未查看」红点在清空统计后
  仍然挂着 —— 现清空统计即视为已知悉全部故障、红点全清；之后运行中新产生的故障照常重新点亮。
- **修复：单个添加主机绕过重复检测。** 批量 / 文件导入路径对列表中已有的主机会弹确认框，
  但「添加主机」的**单个添加**路径直接入库、无声产生重复行 —— 现两条路径走**同一套**
  重复检测与确认交互（大小写、首尾空白变体同样命中）。
- 配置文件新增两个字段（缩放、字体），**旧配置文件升级零丢失**；非法值自动回落默认。

> 验证方式：静态（tsc / cargo test / 前端单测）+ CDP 端到端实测（徽标单行、红点点亮与清除、
> 125% 缩放生效与菜单定位、字体切换生效、设置对话框免滚动、深色模式），共 49 项前端测试 /
> 122 项 Rust 测试通过；用户配置与事件文件测后逐字节还原一致。
